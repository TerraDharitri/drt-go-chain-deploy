...
import (
	...
	"github.com/TerraDharitri/drt-go-chain-deploy/data"
	"github.com/TerraDharitri/drt-go-chain-deploy/generate/disabled"
	drtData "github.com/TerraDharitri/drt-go-chain/genesis/data"
...
func (dsg *delegatedStakingGenerator) computeInitialAccounts(
...
) []drtData.InitialAccount {
	initialAccounts := make([]drtData.InitialAccount, 0, len(walletKeys)+len(additionalKeys))

	for _, key := range delegators {
		delegatorAddress, _ := dsg.walletPubKeyConverter.Encode(key.PubKeyBytes)
		delegatedAddress, _ := dsg.walletPubKeyConverter.Encode(key.DelegatedPubKeyBytes)

		account := drtData.InitialAccount{
			Address:      delegatorAddress,
			Supply:       big.NewInt(0).Add(key.Balance, key.DelegatedValue),
			Balance:      big.NewInt(0).Set(key.Balance),
			StakingValue: big.NewInt(0),
			Delegation: &drtData.DelegationData{
	...
	for _, key := range walletKeys {
		walletAddress, _ := dsg.walletPubKeyConverter.Encode(key.PubKeyBytes)
...
func (dsg *delegatedStakingGenerator) computeInitialNodes(validators []*data.BlsKey) []*sharding.InitialNode {
	...
	for _, blsKey := range validators {
		validatorAddress, _ := dsg.validatorPubKeyConverter.Encode(blsKey.PubKeyBytes)
